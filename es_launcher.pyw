...
from os.path import exists, isdir, isfile, join, splitext, dirname
...
def main():
    ...
    emuls_root = set_emuls_root(emuls_root, '..\\EMULATORS')  # If in the same partition volume can be used relative
    roms_root = set_roms_root(roms_root, '..\\ROMS')          # paths to es.exe
    ...
    if play:
        ...
        if assync_load:  # Loads ES in background while playing video in foreground
            if play_vlc_embedded:
                ...
                startup_videos(vlc_path, intro_file, startup_dir, splashs_dir, boot_videos,
                               play_vlc_embedded, emustation.get_first_hwnd_from_pid()).wait()
...
def startup_videos(vlc_path, intro_file, startup_dir, splashs_dir, boot_videos, play_vlc_embedded=False, hwnd2drawon=0):
    vlcargs = ' --fullscreen --qt-fullscreen-screennumber=1 --video-on-top --no-video-title-show --no-video-deco' \
              ' --ignore-config --reset-config --one-instance --play-and-exit --high-priority --no-lua' \
              ' -Idummy --no-mouse-events --mouse-hide-timeout=1 --file-caching=1000 --plugins-cache'
    if play_vlc_embedded:
        vlcargs += ' --drawable-hwnd=' + str(hwnd2drawon)

    def play_on_vlc(vlc_path, vlcargs, videofiles_dir):
        def pickafile(dir_path_in, v_ext_list=['.mp4', '.m4v', '.mkv', '.flv', '.avi', '.wmv', '.mpg', '.mpeg']):
...
<config>
	...
	<system>
		...
		<command>{emuls_root}\\retroarch\\retroarch.exe -L {emuls_root}\\retroarch\\cores\\picodrive_libretro.dll "%ROM_RAW%"</command>
	...
	<system>
		...
		<command>{emuls_root}\\retroarch\\retroarch.exe -L {emuls_root}\\retroarch\\cores\\flycast_libretro.dll "%ROM_RAW%"</command>
	...
	<system>
		...
		<extension>.7z .sfc .smc .zip .7Z .SFC .SMC .ZIP</extension>
		<command>{emuls_root}\\retroarch\\retroarch.exe -L {emuls_root}\\retroarch\\cores\\snes9x_libretro.dll "%ROM_RAW%"</command>
	...
	<system>
		...
		<extension>.7z .zip .7Z .ZIP</extension>
		<command>{emuls_root}\\retroarch\\retroarch.exe -L "{emuls_root}\\retroarch\\cores\\px68k_libretro.dll" -e "%ROM_RAW%"</command>
	...
	<system>
		...
		<extension>.vbs .bat .exe .pyw .py .VBS .BAT .PYW .PY .EXE</extension>
		<command>"%ROM_RAW%"</command>
...
def check_7zip(path, temp_dir='7zip_files'):
    if not exists(join(path, '7za.exe')):
        ...
        rmtree(temp_dir_path)
        remove(file_path_1)
        sp.run(f'7za.exe x "{file_path_2}" -o"{temp_dir_path}" -y')  # Using 7zip command line
...
def check_emustation_setup(es_path_in):
    if isfile(es_path_in):
        ...
        extract_contents2(file_path, dest_dir=dirname(es_path_in))
...
def check_splashs_setup(path):
    ...
    if isdir(path):
        if checkfiles(path):
            ...
            # extract_contents2(file_path, dest_dir=path)
...
def check_vlc_setup(vlc_path_in):
    ...
    if isfile(vlc_path_in):
        if not exists(join(vlc_dir, 'plugins\\plugins.dat')):
            sp.run(f'"{join(vlc_dir, "vlc-cache-gen.exe")}" plugins', startupinfo=new_startupinfo(window_hidden=True))
        return vlc_path_in
    else:
        ...
        extract_contents2(file_path, dest_dir=dirname(vlc_path_in))
        if not exists(join(vlc_dir, 'plugins\\plugins.dat')):
            sp.run(f'"{join(vlc_dir, "vlc-cache-gen.exe")}" plugins', startupinfo=new_startupinfo(window_hidden=True))
...
def check_themes_setup(path, settings_cfg_file_in):
    if not isdir(path):
        ...
        extract_contents2(file_path, dest_dir=join(path, 'RVGM-BT-Theme'))
        return check_themes_setup(path, settings_cfg_file_in)
    else:
        return path
