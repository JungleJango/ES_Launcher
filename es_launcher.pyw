...
from os import remove, environ, chdir, getcwd, scandir, makedirs, mkdir, listdir
...
def main():
    ...
    if not isdir(es_dir) or es_dir == '':
        if getattr(sys, 'frozen', False):
            environ['ESdir'] = es_dir = dirname(sys.executable)  # if converted to executable this will be used
        elif __file__:
            environ['ESdir'] = es_dir = dirname(__file__)
    chdir(es_dir)
    check_7zip(es_dir)
...
def check_vlc_setup(vlc_path_in):
    if isfile(vlc_path_in):
        ...
        check_button(f'VLC files not found on "{vlc_path_in}"! Will be downloaded now. All credits goes to '
                     f'VLC devs team.')
        download_binary_r('https://get.videolan.org/vlc/3.0.16/win32/vlc-3.0.16-win32.zip',
                          'VLC.zip')
...
def check_themes_setup(path, settings_cfg_file_in, randomize):
    ...
    if not isdir(path):
        ...
        download_binary_r('https://github.com/Darknior/RVGM-BT-Theme/archive/refs/tags/1.96.zip', name + '.zip')
        extract_contents2(name + '.zip', dest_dir=join(path, name))
        check_themes_setup(path, settings_cfg_file_in, randomize)
    if randomize:
